...
# bazzite-os: o payload e baseado em bazzite-nvidia-open, mas o nome da imagem
# nao contem "-nvidia", entao os testes `$imageref == *-nvidia*` do upstream nao
# disparariam. A sessao live roda nouveau e so a instalacao leva o driver
# proprietario, igual a ISO oficial do bazzite-nvidia-open.
is_nvidia=1
...
if [[ $imageref == *-nvidia* || $is_nvidia -eq 1 ]]; then
